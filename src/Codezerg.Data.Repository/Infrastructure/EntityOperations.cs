...
using Codezerg.Data.Repository.Infrastructure.Services;
...
{
    private readonly PrimaryKeyService<T> _primaryKeyService;
    private readonly IdentityManager<T> _identityManager;
    
    public EntityOperations()
    {
        _primaryKeyService = new PrimaryKeyService<T>();
    ...
    {
        EntityUpdateService<T>.UpdateEntityProperties(existingEntity, newEntity, _primaryKeyService.PrimaryKeyProperties);
    }
    ...
    {
        return DeepCopyService<T>.CreateDeepCopy(entity);
    }
}